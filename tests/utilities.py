...
def to_tensor(array: Union[Sequence, np.ndarray]) -> np.ndarray:
    """
    Convert a Python list or NumPy array to a NumPy array.
    - Float arrays become np.float32.
    - Integer arrays become np.int64.
    - NumPy arrays are returned with appropriate dtype.
    """
...
def to_long(array: Union[Sequence, np.ndarray]) -> np.ndarray:
    """Convert a Python list or NumPy array to np.int64 array."""
...
def equal_tensors(x: np.ndarray, y: np.ndarray) -> bool:
    """Check if two arrays are exactly equal."""
    return np.array_equal(x, y)


def almost_equal(a: Union[float, int, np.ndarray],
...
def all_positive(X: np.ndarray) -> bool:
...
def randn(*shape: int) -> np.ndarray:
    """Return a random normal array of given shape."""
    return np.random.randn(*shape).astype(np.float32)


def rand(*shape: int) -> np.ndarray:
    """Return a uniform random array in [0,1) of given shape."""
...
def assert_tensors_are_close(name1: str, X1: np.ndarray,
                             name2: str, X2: np.ndarray,
